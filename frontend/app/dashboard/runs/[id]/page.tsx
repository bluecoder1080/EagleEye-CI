        ...
        <div className="glass-card">
          ...
          <div className="rounded-lg bg-surface p-4 font-mono text-xs">
            {run.formattedFailures.map((line, idx) => (
              <div
                key={idx}
                className="py-1 text-gray-300 border-b border-surface-border last:border-0"
              >
                {line}
              </div>
            ))}
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-surface-border bg-surface-raised p-4">
      <div
        className={`mb-3 inline-flex rounded-lg border p-2 ${colors[color]}`}
      >
        {icon}
      </div>
    ...
    <div className="relative flex gap-4 pl-10">
      ...
      <div className="flex-1 rounded-lg border border-surface-border bg-surface-raised/50 p-3">
        <div className="flex items-start justify-between gap-3">
          <span
            className={`inline-block rounded-md border px-2 py-0.5 text-xs font-medium ${getEventStyle(entry.event)}`}
          >
            {entry.event}
          </span>
...
function FixCard({
  ...
  return (
    <div
      className={`rounded-lg border p-4 ${fix.fixApplied ? "border-emerald-500/20 bg-emerald-500/5" : "border-red-500/20 bg-red-500/5"}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-xs ${fix.fixApplied ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
            >
              {fix.fixApplied ? "✓" : "✗"}
            </span>
