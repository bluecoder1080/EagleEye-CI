...
const EVENT_LABELS: Record<string, string> = {
  ORCHESTRATOR_START: "Agent started",
  CLONE_START: "Cloning repository",
  CLONE_DONE: "Repository cloned",
  CLONE_FAILED: "Clone failed",
  CLONE: "Repository cloned",
  BRANCH_CREATED: "Fix branch created",
  TEST_RUN_START: "Running tests in sandbox",
  TESTS_RUN: "Running tests",
  TESTS_PASSED: "All tests passed",
  TESTS_FAILED: "Tests failed",
  CLASSIFY_START: "Classifying failures",
  CLASSIFY_DONE: "Failures classified",
  CLASSIFY_REGEX_MISS: "Regex miss — using LLM fallback",
  CLASSIFY_NO_FAILURES: "No failures could be classified",
  CLASSIFIED_FAILURE: "Failure classified",
  FIX_GENERATE_START: "Generating fixes via LLM",
  FIX_GENERATE_DONE: "Fixes generated",
  FIX_GENERATED: "Fix generated by LLM",
  NO_FIXES_GENERATED: "No fixes could be generated",
  PATCH_APPLY_START: "Applying patches",
  PATCH_APPLY_DONE: "Patches applied",
  ...
  ORCHESTRATOR_DONE: "Pipeline completed",
...
function EventDot({ event }: { event: string }) {
  ...
  if (
    ...
    event === "FIX_GENERATED" ||
    event === "FIX_APPLIED" ||
    event === "FIX_GENERATE_START" ||
    event === "FIX_GENERATE_DONE" ||
    event === "PATCH_APPLY_START" ||
    event === "PATCH_APPLY_DONE"
  ) {
    color = "bg-brand-500";
  } else if (
    event === "CLASSIFIED_FAILURE" ||
    event === "CLASSIFY_START" ||
    event === "CLASSIFY_DONE" ||
    event === "CLASSIFY_REGEX_MISS"
  ) {
    color = "bg-yellow-500";
  } else if (
    event === "COMMITTED" ||
